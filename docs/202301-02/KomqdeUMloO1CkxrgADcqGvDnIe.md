...
*   在Gdrive根目录下创建stablediffusion文件夹，然后进入之后再创建models文件夹

*   将所有下载好的模型文件都丢到/stablediffusion/models/这个文件夹里边

![](img/d08904531783dab13f6ff67ad2aad575.png)

*   colab中执行以下拷贝命令，将Gdrive文件拷贝到colab环境中
...
*   进入主页创建一个新的model，Model name和License随便填，注意，该model选为Private私有的。这里的原因是，有些model是泄露出的，并不是官方开源的，避免一些麻烦，所以你还没玩懂之前，选成私有的自己玩玩。

![](img/6b3262a36beab473b1e98a34165698ec.png)

*   进入到刚才创建好的model中，选择Files and versions选项卡，点击右边的Add files->upload files，随后把自己下载好的模型文件都丢进来上传上去，上传完成之后点左下角的commit，成功之后如下图

![](img/79001db5662c47feff07a25f927da3a9.png)

*   使用的时候点击上图模型文件名称，进入后右键选择拷贝link地址，就获取到了这个模型文件的下载链接。

![](img/113ec0e357110a98bad1e431d6115762.png)

*   生成token，进入下方token获取地址，如下图生成一个token，这个token作用就是colab访问huggingface下载文件的时候做权限验证用的。

地址：https://huggingface.co/settings/tokens

![](img/16a4d051fb85bf8cac824f2714143400.png)

*   将上述步骤生成的文件下载链接以及token填入脚本中的输入框，colab运行的时候就可以正常访问并且下载该文件了。
