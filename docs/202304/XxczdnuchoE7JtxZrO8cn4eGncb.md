...
1.  这里大家可以直接复制照搬我的，但我强烈建议自己总结一套独属自己的方法

1.  总结方法其实是简单的，但为什么标题还老是取不好？往往都是自己理解不够到位，没有真的按照方法去写。正好GPT就是这样的执行机器，并且你还可以让他瞬间写出10个标题，到时候你再自己慢慢挑嘛。

1.  方法不要多，一个就好。如果你用GPT多了，你会发现给多了反而不好。如果不想用这个，换个方法再训练一个GPT。
...
1.  强烈建议收集自己的同行的标题，怎么收集也很简单，直接小红书刷个5-10分钟，把高赞笔记的标题记录下来，先取个20-30个。然后再选10个左右，你自己看到都忍不住想点开的标题
...
1.  GPT现阶段最适合作为生产力工具，而不是百科全书或是任何一类聊天机器人。

1.  GPT入门教程的价值会随着信息差填平而变得一文不值，真正有价值的课程是面向创作者、提高其生产效率的落地化专业课程。

1.  GPT是一款工具，GPT本身极难变现，能够变现的是项目本身，GPT只是一项极大提高投产比的生产资料。（那些卖号的不要跟我杠，你们是牛的。）

1.  GPT是一名3岁的天才神童，伤仲永的本质是当爹的教育失败。

1.  训练GPT，把自己当成一名老师而不是老板。

1.  GPT4.0患上了阿兹海默症，所以不断修正有时候会比一次性投喂更好。（这句话能懂的基本都是对话式训练的实战应用派了）

1.  GPT提示词集，确实是值得拿来卖钱的。（注意是词集）
